...
#        return ManageAppointments(address, activity,initHour).makeAppointment(address)
        ...
        subs = db.session.query(Appointment).join('enrolled','user').filter(User.name==participant.name).filter(Appointment.activity==activity).filter(Appointment.initHour==initHour).first()
